...
response = urllib.request.urlopen(url= 'https://movie.douban.com/j/search_tags?type=movie')
...
for tag in tags:
    ...
    start = 0
    while True:
        url = "https://movie.douban.com/j/search_subjects?type=movie&tag=" + urllib.parse.quote(tag) + "&page_limit=20&page_start=" + str(start)
        response = urllib.request.urlopen(url=url)
        subjects = json.loads(response.read())['subjects']
        if len(subjects) == 0 :
            break
