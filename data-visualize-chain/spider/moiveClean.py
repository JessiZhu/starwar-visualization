...
fr = open('douban_movie_detail.txt' , 'r' , encoding='utf8')
fw = open('douban_movie_clean.txt' , 'w' , encoding='utf8')
...
for line in fr:
    ...
    for item in district:
        if item.strip() not in nameMap.keys():
            continue
    ...
    showtime = line[11].split('/')[0][:10]

    length = line[12].split('/')
    if  length[0].isdigit():
        length = length[0]
    else:
        for x in list(range(0 , len(length[0]))):
            if length[0][x].isdigit():
                continue
            else:
                break
        length = length[0][:x]
    ...
    record = str(movieId) + '^' + title + '^' + url + '^' + cover + '^' + str(
        rate) + '^' + director + '^' + composer + '^' + actor + '^' + category + '^' + district + '^' + language + '^' + str(
        showtime) + '^' + str(length) + '^' + othername + '^' + description + '\n'
    fw.write(record)
    count = count + 1
fr.close()
fw.close()